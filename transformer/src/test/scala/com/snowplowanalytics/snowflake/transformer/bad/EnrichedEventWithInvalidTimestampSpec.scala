...
class EnrichedEventWithInvalidTimestampSpec extends Specification with TransformerJobSpec {
  ...
  "A job which processes Snowplow enriched events with invalid timestamp" should {
    ...
    "write a bad row JSON with enriched event and error message for each input line" in {
      ...
      jsons.map(parse).sequence mustEqual Right(List(EnrichedEventWithInvalidTimestampSpec.expected))
    }
