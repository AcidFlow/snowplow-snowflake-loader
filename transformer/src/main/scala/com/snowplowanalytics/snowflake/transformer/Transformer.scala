...
object Transformer {
    ...
    EventsManifestSingleton.get(eventsManifestConfig) match {
      case Some(manifest) =>
        if (manifest.put(eventId, eventFingerprint, etlTstamp)) {
          Some((shredTypes, truncateFields(event.toJson(true), atomicSchema).noSpaces))
        } else None
      case None => Some((shredTypes, truncateFields(event.toJson(true), atomicSchema).noSpaces))
    }
  }
  ...
  /**
    ...
    */
  def truncateFields(eventJson: Json, atomicSchema: Schema): Json = {
    Json.fromFields(eventJson.asObject.getOrElse(throw new RuntimeException(s"Event JSON is not an object? $eventJson")).toList.map {
      case (key, value) if value.isString =>
        val maxLength = for {
          properties <- atomicSchema.properties
          property <- properties.value.get(key)
          maxLength <- property.maxLength
        } yield maxLength
        maxLength match {
          case Some(length) => (key, value.asString.getOrElse("").take(length.value.intValue).asJson)
          case None => (key, value)
        }
      case other => other
    })
