...
object TransformerJob {
  ...
  def run(spark: SparkSession, manifest: ProcessManifest, tableName: String, jobConfigs: List[TransformerJobConfig], eventsManifestConfig: Option[EventsManifestConfig], inbatch: Boolean, atomic: Map[String, Option[Int]]): Unit = {
    jobConfigs.foreach { jobConfig =>
      ...
      val shredTypes = process(spark, jobConfig, eventsManifestConfig, inbatch, atomic)
  ...
  /**
    ...
    * @return list of discovered shredded types
    */
  def process(spark: SparkSession, jobConfig: TransformerJobConfig, eventsManifestConfig: Option[EventsManifestConfig], inbatch: Boolean, atomic: Map[String, Option[Int]]) = {
    ...
    val snowflake = dedupedEvents.flatMap { e =>
      Transformer.transform(e, eventsManifestConfig, atomic) match {
